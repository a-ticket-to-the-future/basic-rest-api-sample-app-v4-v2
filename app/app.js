...
// リクエストのbodyをパースする設定
// body-parserは非推奨なので　express.~で対応　Qiita記事（2021年5月頃の記事）参照
...
// Create a new user
app.post('/api/v4/users', async (req,res)=>{
...
// Update user data
app.put('/api/v4/users/:id', async (req,res)=>{
    if(!req.body.name || req.body.name ===""){
        res.status(400).send({error:"ユーザー名が指定されていません"})
    }else{
    // Connect database
    ...
    db.get(`SELECT * FROM users WHERE id=${id}` , async (err,row) => {
        ...
        if (!row) {
            res.status(404).send({error:"指定されたユーザーがみつかりません"})
        } else {
...
// Delete user data
app.delete('/api/v4/users/:id', async (req,res)=>{
